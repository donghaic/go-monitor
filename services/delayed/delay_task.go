...
func NewDelayedQueue(dataFile string) (*DelayedTaskQueue, error) {
	...
	db, err := bolt.Open(fmt.Sprintf("%s/my.db", dataFile), 0600, nil)
	if err != nil {
		return nil, err
	}
